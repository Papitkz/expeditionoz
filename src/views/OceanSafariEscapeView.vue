...
const cms = useComponentCMS('OceanSafariEscapeView')
const tripData = useTripData('ocean-safari-escape')
...
// Vessel gallery — fixed 6 slots; use getSlot() to prevent drift.
// Matches AdminImages: OceanSafariEscapeView / vesselGallery slots 0-5.
...
// Dining gallery — fixed 4 slots; use getSlot() to prevent drift.
// Matches AdminImages: OceanSafariEscapeView / diningGallery slots 0-3.
...
const fallbackItinerary: ItineraryItem[] = [
  {
    day: 'Day 1',
    title: 'Departure & First Anchorage',
    description: 'Board Sylfia at Exmouth Marina at 3pm. Brief safety orientation, then set sail for a remote anchorage. Afternoon snorkel and sunset dinner on deck.',
    imageUrl: 'https://images.unsplash.com/photo-1544551763-46a013bb70d5?auto=format&fit=crop&w=1200&q=80',
  },
  {
    day: 'Day 2',
    title: 'Remote Reefs & Wildlife',
    description: 'Wake up anchored on the reef. A full day exploring remote sections of Ningaloo — multiple snorkel sessions, wildlife encounters and a beach visit.',
    imageUrl: 'https://images.unsplash.com/photo-1583212292454-1fe6229603b7?auto=format&fit=crop&w=1200&q=80',
  },
  {
    day: 'Day 3',
    title: 'Coral Gardens & Sunset Anchorage',
    description: 'Drift over spectacular coral formations and look out for turtles, reef sharks and rays in the shallows. Sail to a new anchorage for a final sunset dinner under the stars.',
  ...
  },
  {
    ...
    description: 'One last sunrise swim before heading back to Exmouth. Farewell brunch on deck and a return to the marina by 11am.',
    imageUrl: 'https://images.unsplash.com/photo-1560275619-4662e36fa65c?auto=format&fit=crop&w=1200&q=80',
  },
]

// Itinerary — fixed 4 slots; use getSlot() per index to prevent day-image drift.
// Matches AdminImages: OceanSafariEscapeView / itinerary slots 0-3.
const ITINERARY_SLOTS = 4
...
useSEO({
  title: 'Ocean Safari Escape – 4-Day Ningaloo Sailing Escape',
  description: 'Ocean Safari Escape: 3-night Ningaloo sailing escape aboard Sylfia for up to 12 guests. Remote reefs, wildlife, beach visits and sunset anchorages. From $3,000 AUD. Departing Exmouth, WA.',
  path: '/expeditions/ocean-safari-escape',
  type: 'product',
  jsonLd: {
    "@context": "https://schema.org",
    "@type": "Product",
    "name": "Ocean Safari Escape — 4-Day Ningaloo Sailing Escape",
    "description": "Short, immersive Ningaloo sailing escape aboard Sylfia. Maximum 12 guests, remote reefs, wildlife and sunset anchorages, all-inclusive.",
  ...
  <div>
    ...
    <section class="relative h-[85vh] md:h-screen w-full overflow-hidden">
      ...
      <div class="relative z-10 h-full flex flex-col justify-center items-center text-center px-4">
        <p class="overline-text mb-3 md:mb-4 text-xs md:text-sm tracking-[0.3em] text-white/90">4 Day Live-Aboard Escape</p>
        ...
        <p class="font-display text-xl md:text-4xl italic text-[#C9A84C] mb-4 md:mb-6" style="font-family: var(--font-display);">
          A Ningaloo Sailing Escape
        </p>
        <p class="max-w-2xl text-sm md:text-lg text-white/90 mb-6 md:mb-8 font-light leading-relaxed px-2">
          A short, immersive Ningaloo expedition aboard Sylfia — remote reefs, wildlife, beaches and sunset anchorages in three nights.
        </p>
        <div class="flex gap-3 md:gap-4">
          <router-link to="/book/ocean-safari-escape" class="btn-primary px-6 py-3 md:px-8 md:py-4 text-sm md:text-base">
            Check Availability
          </router-link>
        </div>
      </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-12 lg:gap-16 items-start">
          <div class="section-reveal-left order-2 lg:order-1">
            ...
            <h2 class="font-display text-2xl md:text-4xl lg:text-5xl font-light mb-3 md:mb-6" style="font-family: var(--font-display); color: var(--color-sand-100);">
              A Taste of <span class="italic" style="color: var(--color-gold-400);">Life at Sea</span>
            </h2>
            <p class="text-sm md:text-base leading-relaxed mb-3 md:mb-5 opacity-80" style="font-family: var(--font-body); color: var(--color-sand-200); line-height: 1.9;">
              The Ocean Safari Escape is designed for guests who want a concentrated taste of Ningaloo without committing to a longer voyage. Three carefully curated days, maximum 12 guests, and all the luxury you deserve aboard Sylfia.
            </p>
            <p class="text-sm md:text-base leading-relaxed mb-4 md:mb-8 opacity-80" style="font-family: var(--font-body); color: var(--color-sand-200); line-height: 1.9;">
              Perfect for couples, friends, families and travellers already visiting Exmouth who want to experience Ningaloo beyond the day boats — remote reefs, wildlife encounters, beach visits and sunset anchorages, shaped each day by the best conditions on offer.
            </p>

            <div class="grid grid-cols-3 gap-2 md:gap-6 mb-4 md:mb-8 pt-3 md:pt-6" style="border-top: 1px solid rgba(201, 168, 76, 0.15);">
              <div class="text-center">
                <p class="font-display text-xl md:text-3xl font-light" style="font-family: var(--font-display); color: var(--color-gold-400);">{{ tripData.durationDays.value || 4 }}</p>
                <p class="overline-text mt-1 text-[0.5rem] md:text-[0.55rem]">Days</p>
              </div>
              <div class="text-center">
                <p class="font-display text-xl md:text-3xl font-light" style="font-family: var(--font-display); color: var(--color-gold-400);">{{ tripData.maxGuests.value || 12 }}</p>
            ...
            <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3 md:gap-4 mt-1">
              <router-link to="/book/ocean-safari-escape" class="btn-primary inline-block text-sm md:text-base px-5 py-3 md:px-8 md:py-4">
                Reserve Your Cabin
              </router-link>
            ...
            <div class="relative overflow-hidden h-[250px] sm:h-[350px] md:h-[520px] group">
              <template v-if="aboutItem?.imageUrl">
                <img
                  ...
                  :alt="aboutItem.alt || 'Ocean Safari expedition image'"
    ...
    <section class="py-12 md:py-24 lg:py-32" style="background: var(--color-ocean-950);">
      <div class="container mx-auto px-4 sm:px-6 lg:px-12 max-w-6xl">
        ...
        <div class="text-center mt-12 md:mt-20 pt-8 md:pt-12 border-t border-[#C9A84C]/20 section-reveal">
          ...
          <router-link to="/book/ocean-safari-escape" class="btn-primary px-8 py-3 md:px-12 md:py-4 text-sm md:text-base">
            Book Expedition
          </router-link>
        </div>
      </div>
        ...
        <div class="max-w-4xl mx-auto bg-[#0A2E4A]/80 backdrop-blur-md p-4 md:p-12 border border-[#C9A84C]/20 section-reveal">
          <div class="flex flex-col md:flex-row justify-between items-center gap-4 md:gap-6 relative">
            ...
            <div class="relative z-10 text-center flex md:block items-center gap-3 md:gap-0 w-full md:w-auto">
              <div class="w-2 h-2 bg-[#C9A84C] md:mx-auto md:mb-2 flex-shrink-0" />
              <div class="text-left md:text-center">
                <p class="font-display text-base md:text-lg text-white">Remote Reef</p>
                <p class="text-[10px] md:text-xs opacity-60" style="color: var(--color-sand-200);">Wildlife & Snorkelling</p>
              </div>
            </div>
            ...
            <div class="relative z-10 text-center flex md:block items-center gap-3 md:gap-0 w-full md:w-auto">
              <div class="w-2 h-2 bg-[#C9A84C] md:mx-auto md:mb-2 flex-shrink-0" />
              <div class="text-left md:text-center">
                <p class="font-display text-base md:text-lg text-white">Remote Beach</p>
