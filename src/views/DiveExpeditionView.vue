...
const cms = useComponentCMS('DiveExpeditionView')
const tripData = useTripData('dive-expedition')
...
const fallbackItinerary: ItineraryItem[] = [
  {
    ...
    description: 'Board the M.Y. Dive Expedition at Exmouth Marina. Welcome briefing, safety orientation, and departure. First sunset dinner on the upper deck as we sail north to our first anchorage.',
    imageUrl: 'https://images.unsplash.com/photo-1544551763-46a013bb70d5?auto=format&fit=crop&w=1200&q=80',
  },
  {
    day: 'Day 2',
    title: 'Northern Reef Exploration',
    description: 'Wake up anchored on the northern reef. Full day of snorkel sessions over untouched coral gardens. Evening marine biology presentation on deck.',
  ...
  },
  {
    ...
    description: 'The day you\'ve been waiting for. Our spotter plane locates whale sharks and we enter the water for multiple swims alongside these gentle giants. You will be changed forever.',
    imageUrl: 'https://images.unsplash.com/photo-1719450589784-c2c36ccf8e5b?q=80&w=1075&auto=format&fit=crop',
  },
  {
    day: 'Day 4',
    title: 'Scuba Diving & Deep Reef',
    description: 'Full day scuba diving on deeper reef walls and channels. Explore coral bommies, caves, and passages. Evening RIB adventure to a remote beach for sundowners.',
    imageUrl: 'https://images.unsplash.com/photo-1616464592706-f39e5b192451?q=80&w=1633&auto=format&fit=crop',
  },
  {
    day: 'Day 5',
    title: 'Humpback Whale Swim (Seasonal)',
    description: 'Dedicated humpback whale encounter day. Our crew tracks pods and we enter the water for in-water encounters with these magnificent animals — one of the rarest wildlife experiences on earth.',
  ...
  },
  {
    ...
    description: 'A full day at Turquoise Bay — Ningaloo\'s most iconic snorkel site. Drift over spectacular coral formations. Encounter sea turtles, reef sharks, and rays in the crystal-clear shallows.',
    imageUrl: 'https://images.unsplash.com/photo-1437622368342-7a3d73a34c8f?auto=format&fit=crop&w=1200&q=80',
  },
  {
    day: 'Day 7',
    title: 'Expedition RIB Adventures',
    description: 'Dedicated RIB day — explore remote coastline, hidden lagoons, and off-map anchorages unreachable by day boats. Kayak Yardie Creek in the afternoon, stargazing dinner at anchor.',
  ...
  },
  {
    ...
    description: 'Sail south to Coral Bay for manta ray encounters. These graceful creatures glide through the water just meters from you. Farewell dinner under the stars at anchor.',
  ...
  },
  {
    ...
    description: 'One last sunrise swim on the reef before returning to Exmouth. Farewell brunch on deck, certificate presentation, and a lifetime of extraordinary memories.',
  ...
  <div>
    ...
    <section class="relative h-[85vh] md:h-screen w-full overflow-hidden">
      ...
      <div class="relative z-10 h-full flex flex-col justify-center items-center text-center px-4">
        <p class="overline-text mb-3 md:mb-4 text-xs md:text-sm tracking-[0.3em] text-white/90">9 Day Luxury Live-Aboard</p>
        ...
        <p class="max-w-2xl text-sm md:text-lg text-white/90 mb-6 md:mb-8 font-light leading-relaxed px-2">
          Our flagship week-long expedition aboard the M.Y. Dive Expedition. Nine days, eight nights, and the most comprehensive Ningaloo Reef experience available.
        </p>
        <div class="flex gap-3 md:gap-4">
          <router-link to="/book/dive-expedition" class="btn-primary px-6 py-3 md:px-8 md:py-4 text-sm md:text-base">
            Reserve Your Suite
          </router-link>
        </div>
      </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-12 lg:gap-16 items-start">
          <div class="section-reveal-left order-2 lg:order-1">
            ...
            <p class="text-sm md:text-base leading-relaxed mb-3 md:mb-5 opacity-80" style="font-family: var(--font-body); color: var(--color-sand-200); line-height: 1.9;">
              The Dive Expedition is our most ambitious expedition — nine days that will transform how you see the ocean. With luxury suites, world-class dining, and an itinerary designed by marine biologists, this is the ultimate Ningaloo experience.
            </p>
            <p class="text-sm md:text-base leading-relaxed mb-4 md:mb-8 opacity-80" style="font-family: var(--font-body); color: var(--color-sand-200); line-height: 1.9;">
              From sunrise yoga on deck to midnight stargazing over the Indian Ocean — nine days anchored in one of earth's last intact reef systems. The kind of place that recalibrates you.
            </p>

            <div class="grid grid-cols-3 gap-2 md:gap-6 mb-4 md:mb-8 pt-3 md:pt-6" style="border-top: 1px solid rgba(201, 168, 76, 0.15);">
              <div class="text-center">
                <p class="font-display text-xl md:text-3xl font-light" style="font-family: var(--font-display); color: var(--color-gold-400);">{{ tripData.durationDays.value || 9 }}</p>
              ...
              <div class="text-center">
                ...
                <p class="overline-text mt-1 text-[0.5rem] md:text-[0.55rem]">Private Suites</p>
              </div>
            </div>

            <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3 md:gap-4 mt-1">
              <router-link to="/book/dive-expedition" class="btn-primary text-center text-sm md:text-base px-5 py-3 md:px-8 md:py-4">
                Reserve Your Suite
              </router-link>
                  ...
                  <template v-for="(img, i) in [aboutImages[0], aboutImages[2]]" :key="i">
                    <div v-if="img" class="overflow-hidden flex-1" :class="i === 0 ? 'h-[calc(50%-4px)]' : 'h-[calc(50%-4px)]'">
                      <template v-if="img.hasImage">
                        <img
                          :src="img.src"
                          :alt="img.alt || 'Dive Expedition expedition image'"
      ...
      <div class="container mx-auto px-4 sm:px-6 lg:px-12">
        <div class="text-center mb-8 md:mb-16 section-reveal">
          ...
          <h2 class="font-display text-2xl md:text-4xl lg:text-5xl font-light" style="font-family: var(--font-display); color: var(--color-sand-100);">
            M.Y. <span class="italic" style="color: var(--color-gold-400);">Dive Expedition</span>
          </h2>
          <p class="mt-3 md:mt-4 max-w-2xl mx-auto text-sm md:text-base opacity-70 px-2" style="color: var(--color-sand-200);">
            Purpose-built for expedition cruising, the Dive Expedition combines the stability and range of a commercial vessel with the elegance of a private yacht.
          </p>
        </div>
      ...
      <div class="container mx-auto px-4 sm:px-6 lg:px-12 max-w-6xl">
        ...
        <div class="text-center mb-12 md:mb-20 lg:mb-24 section-reveal">
          ...
          <p class="mt-3 md:mt-4 max-w-xl mx-auto text-sm md:text-base opacity-70 px-2" style="color: var(--color-sand-200);">
            Nine carefully designed days that build in intensity — from gentle introductions to extraordinary encounters.
          </p>
        </div>
        ...
        <div class="text-center mt-12 md:mt-20 pt-8 md:pt-12 border-t border-[#C9A84C]/20 section-reveal">
          ...
          <router-link to="/book/dive-expedition" class="btn-primary px-8 py-3 md:px-12 md:py-4 text-sm md:text-base">
            Book Expedition
          </router-link>
        </div>
      </div>
        ...
        <div class="grid lg:grid-cols-2 gap-6 md:gap-12 items-center">
          <div class="section-reveal-left">
            ...
            <h2 class="font-display text-2xl md:text-4xl lg:text-5xl font-light mb-4 md:mb-6" style="font-family: var(--font-display); color: var(--color-sand-100);">
              Dining at <span class="italic" style="color: var(--color-gold-400);">Dive Expedition</span>
            </h2>
            <p class="text-sm md:text-base leading-relaxed mb-4 md:mb-6 opacity-80" style="font-family: var(--font-body); color: var(--color-sand-200); line-height: 1.9;">
              With a dedicated chef and premium provisions sourced from across Western Australia, dining aboard the Dive Expedition is an integral part of the experience — not an afterthought.
            </p>
